...
#query = generate_query.invoke({"question": "what phone has the best camera?"})
...
#print(final_prompt.format(input="What is the best camera",table_info=db,messages=[], top_k = 5))
...
question = "What is the best screen?"

#memory.chat_memory.add_user_message(question)
history.add_user_message(question)

#history.messages[HumanMessage(content='How many cutomers with order count more than 5'),AIMessage(content='There are 2 customers with an order count of more than 5.')]
response = chain.invoke({"question": question,"messages":history.messages})
print(response) 
history.add_ai_message(response)
#memory.chat_memory.add_ai_message(response)
###########now just figure proper memory

print(memory.load_memory_variables({})) 

Question2 = "What year did it come out?"
history.add_user_message(Question2)
#memory.chat_memory.add_user_message(Question2)

#history.messages[HumanMessage(content='How many cutomers with order count more than 5'),AIMessage(content='There are 2 customers with an order count of more than 5.')]
response = chain.invoke({"question": Question2,"messages":history.messages})
print(response) 
history.add_ai_message(response)
#memory.chat_memory.add_ai_message(response)
###########now just figure proper memory
...
Question3 = "How good is its battery life??"
history.add_user_message(Question3)
#memory.chat_memory.add_user_message(Question2)

#history.messages[HumanMessage(content='How many cutomers with order count more than 5'),AIMessage(content='There are 2 customers with an order count of more than 5.')]
response = chain.invoke({"question": Question3,"messages":history.messages})
print(response) 
history.add_ai_message(response)
#memory.chat_memory.add_ai_message(response)
###########now just figure proper memory
...
# notes had to go into get_buffer_string which is in FewShotChatMessagePromptTemplate -> FewShotChatMessagePromptTemplate -> Change
# The AI: prefeix to str = "/*AI:*/" as this makes a commented out and closed new line 
